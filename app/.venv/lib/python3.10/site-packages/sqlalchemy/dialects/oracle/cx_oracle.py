...
r""".. dialect:: oracle+cx_oracle
    :name: cx-Oracle
    :dbapi: cx_oracle
    :connectstring: oracle+cx_oracle://user:pass@hostname:port[/dbname][?service_name=<services>[&key=value&key=value...]]
...
Given a hostname, port and services name of the target database, for example
...
Alternatively, if no port, database name, or services name is provided, the
...
The cx_Oracle dialect connects to this database services when ``myalias`` is the
...
services name required by Easy Connect.  The Easy Connect syntax does not
support SIDs.
