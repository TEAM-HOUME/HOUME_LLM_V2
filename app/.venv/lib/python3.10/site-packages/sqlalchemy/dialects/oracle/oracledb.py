...
r""".. dialect:: oracle+oracledb
    ...
    :dbapi: oracledb
    :connectstring: oracle+oracledb://user:pass@hostname:port[/dbname][?service_name=<services>[&key=value&key=value...]]
...
Given the hostname, port and services name of the target database, you can
...
If no port, database name, or services name is provided, the dialect will use an
...
The python-oracledb dialect connects to this database services when ``myalias`` is the
